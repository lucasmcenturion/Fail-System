...
bool deadlockRecursivo(char* inicial, t_list* listaDL, esiBloqueado* esi, int* iteracion)
{
	if(6 > iteracion[0])
	{
		clavexEsi* cxe = list_find(clavesBloqueadas, LAMBDA(bool _(clavexEsi* item1){ return !strcmp(item1->clave, esi->clave);}));
		if(cxe != NULL)
		{
...
void Deadlock()
{
	...
	list_iterate(deadlocks,LAMBDA(void _(t_list* item1){
		list_sort(item1, LAMBDA(bool _(char* i1, char*i2){
				return i1 < i2;
			}));
	}));
	for (i=0; i< list_size(deadlocks); i++)
	{
		...
		list_remove_by_condition(deadlocks, LAMBDA(bool _(t_list* item1){
			if (list_size(item1) == list_size(deadlock))
			{
				bool rv = true;
				int k=0;
				list_iterate(item1,LAMBDA(void _(char* idDelEsi){
					if(rv)
						if (strcmp(idDelEsi, list_get(deadlock,k)))
							rv = false;
					k++;
				}));
				return rv;
