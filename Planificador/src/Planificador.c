			...
			case MUERTEESI: {
				...
				while (list_any_satisfy(clavesBloqueadas,
					...
					if (esiADesbloquear != NULL) {
						...
						clavexEsi *clavexEsiAAgregar = malloc(
								sizeof(clavexEsi));
						clavexEsiAAgregar->clave = malloc(
								strlen(clavexEsiABorrar->clave) + 1);
						strcpy(clavexEsiAAgregar->clave,
								clavexEsiABorrar->clave);
						clavexEsiAAgregar->idEsi = malloc(
								strlen(esiADesbloquear->esi->id) + 1);
						strcpy(clavexEsiAAgregar->idEsi,
								esiADesbloquear->esi->id);
...
void HacerSJF() {
	...
	t_list *listaConLosMenoresOrdenadosPorOrdenDeLlegada =
			list_filter(LISTOS,
					LAMBDA(
							bool _(procesoEsi * item1) { return item1->rafagasEstimadas == esiMenorEst->rafagasEstimadas; }));
	procesoEsi *esiMenoryPrimeroEnLlegar = list_get(
			listaConLosMenoresOrdenadosPorOrdenDeLlegada, 0);
